...
# noise_variance引数を削除し、etaを直接受け取るように変更
def linear_lasso(X, y, lam=0, beta=None, eta=None):
    # Args:
        ...
        # eta (np.ndarray, optional): zに加えるノイズ. Defaults to None.
    n, p = X.shape
    if beta is None:
        beta = np.zeros(p)
    X, y, X_bar, X_sd, y_bar = centralize(X, y)
    
    # ノイズ eta がNoneの場合は、ノイズなしとして0の配列を使用
    if eta is None:
        eta = np.zeros((p, n))
...
plt.title(f"LASSO Path With Unique Noise (Variance Σ={noise_variance_value}) for California Housing Dataset")
plt.grid(True)
